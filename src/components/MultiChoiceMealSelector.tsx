...
import { Utensils, AlertCircle, Check, X, PlusCircle, Edit3 } from 'lucide-react';
import { CustomOptionPreset } from '../types/canteen';

interface MultiChoiceMealSelectorProps {
  label: string;
  ...
  onToggleEaten: () => void;
  onSelectPreset: (preset: { id?: string; label: string; price: number }) => void;
  onSetCustom: (item: string, price: number) => void;
}

export const MultiChoiceMealSelector: React.FC<MultiChoiceMealSelectorProps> = ({
  label,
  icon,
  eaten,
  selectedItem = '',
  ...
  const [customItem, setCustomItem] = useState(selectedItem);
  const [customPrice, setCustomPrice] = useState(price > 0 ? String(price) : '');

  const isIncomplete = eaten && (!selectedItem || price <= 0);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          <div className="text-right">
            ...
            >
              {eaten ? (isIncomplete ? 'Rs. ?' : `Rs. ${price}`) : 'Rs. 0'}
            </div>
            {eaten && !isIncomplete && (
          ...
          <button
            ...
            aria-label={eaten ? `Mark ${label} skipped` : `Mark ${label} eaten`}
          >
        ...
        <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 space-y-2.5">
          ...
          {isIncomplete && (
            <div className="flex items-center space-x-2 text-xs font-medium text-amber-700 dark:text-amber-300 bg-amber-100/80 dark:bg-amber-950/60 p-2.5 rounded-xl border border-amber-300/60 dark:border-amber-700/60">
              ...
              <span>Please pick an option or enter a custom item for {label}.</span>
            </div>
          )}

          {/* Quick Preset Pills */}
          <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            <span>Choose from options:</span>
            <button
              type="button"
          ...
          {presets.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {presets.map((preset) => {
                const isSelected =
                  selectedItem === preset.label && price === preset.price;
                return (
                  <button
                    key={preset.id}
                    type="button"
            ...
            <p className="text-xs text-slate-400 italic">No preset options configured. Tap Custom to enter.</p>
          )}
